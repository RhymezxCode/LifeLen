...
| `:core:search` | Multi-provider search/shopping grounding. | `SearchClient` (interface), `AggregatingSearchClient` (Google + DuckDuckGo + Bing + optional Serper), `GoogleScrapeSearchClient`, `DuckDuckGoSearchClient`, `BingSearchClient`, `SerperSearchClient`, `SearchResult` |
| `:core:data` | Repositories, use cases, the `CategoryHandler` registry, connectivity, and location. | `ScanRepository`, `HistoryRepository`, `CategoryHandler`, `FoodHandler`, `ElectronicsHandler`, `BookHandler`, `ClothingHandler`, `PlantHandler`, `DocumentHandler`, `GenericHandler`, `CategoryHandlerRegistry`, `NetworkMonitor`, `AndroidRegionProvider` |
...
- **`:core:search`** binds `SearchClient` to `AggregatingSearchClient`, which fans out to keyless Google/DuckDuckGo/Bing scrapes (Google first) plus Serper when `SEARCH_API_KEY` is set, and provides each provider's Retrofit API.
...
- **`:core:data`** binds `ScanRepository`/`HistoryRepository` implementations, contributes the `CategoryHandler` set + `CategoryHandlerRegistry`, and binds `NetworkMonitor` (offline fallback) and `RegionProvider` (`AndroidRegionProvider`, for local-currency pricing).
