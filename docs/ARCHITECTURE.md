...
| `:core:data` | Repositories, use cases, and the `CategoryHandler` registry. | `ScanRepository`, `HistoryRepository`, `CategoryHandler`, `FoodHandler`, `ElectronicsHandler`, `BookHandler`, `ClothingHandler`, `GenericHandler`, `CategoryHandlerRegistry` |
...
graph LR
    ...
    reg -->|ELECTRONICS| elec[ElectronicsHandler]
    reg -->|BOOK| book[BookHandler]
    reg -->|CLOTHING| cloth[ClothingHandler]
...
- Each handler implements a common interface and owns its enrichment:
  ...
  - `ElectronicsHandler` → assemble a spec sheet **and** run the search-grounding → `PriceInfo` pipeline.
  - `BookHandler` → synopsis/metadata; `ClothingHandler` → attributes + optional pricing.
  - `GenericHandler` → the catch-all knowledge/OCR path; guarantees nothing ever falls through.
...
- **`:core:data`** binds `ScanRepository`/`HistoryRepository` implementations and contributes the `CategoryHandler` set + `CategoryHandlerRegistry`.
...
- Events are plain function callbacks passed into the Composable (`onCapture`, `onRetry`, `onFavorite`).
...
- On analyze failure with connectivity loss, the ViewModel surfaces an error state while history/last-result remain browsable — the **offline last-result fallback** (also a demo safety net).
